...
1. **一次只做一个小功能** — 完成后更新 `DONE.md`。
2. **持续更新本文件** — 发现新的注意事项就加进来。
3. **有不明确的需求必须先问我** - 禁止胡乱猜测
...
- **存储占用语义**：当前应用不产生需要用户手动清理的持久化缓存；`audio/`、`covers/` 中数据库仍引用的文件及 `music_pocket/` 数据库属于用户资料，只有经重新核对仍未引用的托管文件可确认后清理。
