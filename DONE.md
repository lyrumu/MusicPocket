...
## ✅ 第 35 步：歌曲左滑加入下一首与歌单管理
- 移除歌曲行的可见“下一首”按钮；资料库、艺术家、歌单和搜索结果统一左滑加入下一首，使用主题色滑动反馈并保留长按菜单入口。
- 滑动内容限制在固定圆角边界内并最多左移 112px；成功后动作区保持展开并显示“✓ 已加入”，再平滑回位。
- 长按菜单精简为“加入歌单 / 加入播放队列 / 编辑信息 / 删除歌曲”，修正歌单与播放队列的图标及术语对应。
- 歌单详情新增“管理 / 完成”按钮，管理状态下通过每首歌曲的移除按钮删除歌单关联，不删除资料库歌曲。
- 验证：新增歌曲行与长按菜单回归测试；`flutter test` 21/21 通过；`flutter analyze` 仅保留原有 2 个 assets 目录警告。

## ✅ 第 36 步：完整播放器队列入口去重
- 移除完整播放器右上角重复的播放队列按钮，保留控制区入口与顶部“查看播放队列”。
- 验证：`flutter test` 21/21 通过；`flutter analyze` 仅保留原有 2 个 assets 目录警告。

## ✅ 第 37 步：缓存入口替换为存储占用
- 确认应用未产生可清理的持久化缓存，移除会停止播放且始终显示 `0 B` 的缓存清理服务。
- 设置页改为展示音频、封面、数据库及总占用。
- 修复异步赋值从 `setState` 回调返回 `Future` 触发断言，以及明细打开前等待目录扫描的问题；对话框立即打开并在内部显示计算进度或错误。
- 新增存储目录统计测试；`flutter test` 22/22 通过；`flutter analyze` 仅保留原有 2 个 assets 目录警告。
